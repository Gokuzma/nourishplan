  ...
  return useQuery({
    ...
    queryFn: async (): Promise<HouseholdWithName | null> => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, household_id, role, households(id, name, week_start_day, weekly_budget, auto_draft_enabled, created_at)')
...
export function useCreateHousehold() {
  ...
  return useMutation({
    mutationFn: async (name: string): Promise<Household> => {
      ...
      return { id: householdId, name, week_start_day: 0, weekly_budget: null, auto_draft_enabled: false, created_at: new Date().toISOString() }
    },
    onSuccess: async () => {
